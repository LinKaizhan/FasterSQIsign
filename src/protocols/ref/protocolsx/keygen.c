...
int protocols_keygen_modified(quat_alg_elem_t *final_beta, ibz_t *final_n_beta, ibz_mat_2x2_t *final_action_matrix, quat_alg_elem_t *final_gen, ibz_t *final_n, public_key_t *pk, secret_key_t *sk) {
    ...
    // setting the public key
    pk->E = sk->curve;
    precompute_for_sign(final_beta, final_n_beta, final_action_matrix, final_gen, final_n, &sk->curve, &sk->basis_minus, &sk->basis_plus, &sk->kernel_dual, &sk->lideal_small, &sk->lideal_two,&sk->gen_two,&QUATALG_PINFTY);
